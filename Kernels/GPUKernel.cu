        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                if (row + i >= 0 && row + i < h && col + j >= 0 && col + j < w) {
                    // check if neighbor pixel is center 
                    if ((i == 0 && j == 0))
                        continue;
...
int main()
{
    ...
    int* d_histo = nullptr; // device pointer for histogram
    int histo[256] = { 0 };  // Host array to store histogram
    int width, height;
    int grey_size;
    ...
    cudaMalloc((void**)&d_histo, 256 * sizeof(int)); // Allocate memory for the histogram

    // Initialize histogram to 0 on the device
    cudaMemset(d_histo, 0, 256 * sizeof(int));

    // create CUDA events (timer to see how much time from gray to LBP)
    ...
    greyscaleToLbpConversion << <GridSize, BlockSize >> > (d_pic_in, d_pic_out, width, height, d_histo);
    ...
    cudaMemcpy(histo, d_histo, 256 * sizeof(int), cudaMemcpyDeviceToHost);
...
// Print histogram as an array
    printf("Histogram of LBP values: [");
    for (int i = 0; i < 256; i++) {
        printf("%d", histo[i]);
        if (i < 255) {
            printf(", ");
